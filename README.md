...
```json
{
    ...
    "cc": "centro-de-costo",
    "s3_bucket": "company-sagemaker-poc-bucket",
...
> ⚠️ `s3_bucket` **debe ser** `company-sagemaker-poc-bucket`. Cualquier otro valor rompe el job `upload-s3`.
...
# 3. Crear el bucket
aws s3 mb s3://company-sagemaker-poc-bucket --endpoint-url http://localhost:4566
...
|-------|-------|----------|
| `Invalid bucket name "{s3-bucket}"` | `config.json` tiene el placeholder sin reemplazar | Poner `"s3_bucket": "company-sagemaker-poc-bucket"` |
